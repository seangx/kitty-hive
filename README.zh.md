...
- peer `status` 只有 ping 成功往返才转 `active`。一直 `inactive` 多半是对方 URL 不可达，或者存的 tunnel URL 过期 —— 看下面 **Tunnel URL 自愈** 和 `npx kitty-hive peer set-url` 手动应急。
...
- **心跳：** `peer add` 当场 ping；server 每 60s 周期 ping 维护 `peers.status`。`npx kitty-hive status` 直接显示。
...
- **文件：** 传输文件落在 `~/.kitty-hive/files/<id>/`，7 天后自动清理。`npx kitty-hive files clean [--days N]` 可手动跑。
...
直接 `npx kitty-hive` 看总览，`npx kitty-hive <group>`（例如 `npx kitty-hive peer`）只看该分组的子命令。在 TTY 下，缺失的参数会以交互式 prompt 方式补齐；脚本里把 flag 都传上即可保持非交互。（如果用 `npm i -g kitty-hive` 装了全局，去掉 `npx ` 前缀即可。）

```
npx kitty-hive serve   [--port 4123] [--db path] [-v|-q]                 启动 MCP server
npx kitty-hive init    [tool] [--port 4123]                              写入 MCP 配置（无 tool 时弹 select）
npx kitty-hive status  [--port 4123]                                     服务/agent/team 状态

npx kitty-hive agent   list | rename [old] [new] | remove [name-or-id]
                       remove --key <K> [--yes]                          按 external_key 幂等删除
                       register --key <K> --display-name <N>             幂等 upsert（stdout = agent_id）
npx kitty-hive peer    invite [--expose <agent>]
                       ...
                       remove  [<name>]
npx kitty-hive tunnel  start  [--port 4123] [--name name]                启动 cloudflared，自动把 URL 注册给 hive
                       status [--port 4123]                              查看当前注册的 tunnel URL
npx kitty-hive config  set    [key] [value]                              设置配置（如 `name`）
npx kitty-hive files   clean  [--days 7]                                 清理过期联邦传输文件
npx kitty-hive db      clear  [--db path]                                清空数据库
npx kitty-hive log     dm    [<agent>] [--limit 50]                      查看涉及某 agent 的 DM 历史
...
任何派生 long-lived 进程的工具（session 管理器、terminal multiplexer、CI runner、IDE shell …）都能让它的子进程以稳定 agent 身份出现在 hive 上：
...
- **[kitty-kitty](https://github.com/seangx/kitty-kitty)** —— 终端 session 管理器。每个由 kitty-kitty 派生的 tmux pane 自动注册为 hive agent（`HIVE_AGENT_KEY = <session uuid>`、`HIVE_AGENT_NAME = <pane 标题>`），pane 销毁时通过 `agent remove --key` 清理对应 agent。上文契约的参考实现。
