...
export function createMcpServer(): McpServer {
  const mcp = new McpServer({
    ...
    instructions: [
      ...
      '',
      '## Team rules',
      'Each team you belong to may have a `rules` field (free-form markdown set by',
      'the team operator via `kitty-hive team rules`). hive_start returns it inside',
      'the teams[] payload; hive_team_info also includes it.',
      '',
      'Treat team.rules as **binding house rules** for that team — they describe',
      'how the operator wants members to behave (naming conventions, when to gate,',
      'who reviews what, etc.). Honor them on every team-scoped action.',
      '',
      'If a team\'s rules conflict with general MCP instructions or your own SKILL,',
      'the team rules win for actions inside that team. If they conflict with the',
      'user\'s explicit request, ask before overriding.',
      '',
      'Rules can change while you\'re running — a `team-rules-update` push will',
      'arrive; re-fetch via hive_team_info(team=X) to read the current text.',
      '',
      '## Team collaboration',
